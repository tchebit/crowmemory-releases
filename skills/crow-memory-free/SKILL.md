...
Entries expire after a TTL (default 72h — push first, start the consumer later). On a queue, `to`/`as` address an entry to a role so agents sharing a folder don't self-pop. A `key` gives latest-value-wins (same-key push supersedes); a keyed push with no payload retracts (tombstone). Put concrete follow-ups in `actions` and unknowns in `unresolved` rather than burying them in `note`. A handoff belongs to the operator who pushed it, where there is an operator to belong to: ownership comes from the licence email, so on the free tier there is no identity, nothing is owned, and every entry is already claimable by anyone on the brain — `shared: true` changes nothing there. On a licensed, multi-operator brain it publishes the entry to everyone. Scope is the licence, not the session: your own second terminal shares your ownership. An empty pop/read explains why, distinguishing "nothing here" from "everything is leased to someone else", and lists the other live channels.
